...
Shared.createMessage = function(props, react_element, timeBeforeClear) {
    ...
    react_element.setState({
        ...
        setTimeout(function() {
            react_element.setState({status_type: "", status_message: ""});
        }, timeBeforeClear);
    });
};
Shared.get_potential_assignees = function(potential_assignees) {
    ...
    $.each(potential_assignees, function(k, v) {
        potential_assignees_sorted.push({key: v.id, value: v.name});
    });
    ...
    var custom_controls = custom_field_attributes.map(function(custom_field) {
        ...
        switch (custom_field.field_type) {
            case 1: //input
                ...
                break;
            case 2: //textarea
                control = R.textarea({id: custom_field.name, name: custom_field.name, placeholder: custom_field.label, value: value, onChange: changeHandler});
                break;
            case 3: //select
